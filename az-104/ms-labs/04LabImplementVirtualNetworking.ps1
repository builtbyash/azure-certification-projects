...
# Create an NSG and associate it with a subnet

New-AzApplicationSecurityGroup -ResourceGroupName "az104-rg4" -Location "eastus" -Name "asg-web"
$nsg = New-AzNetworkSecurityGroup -ResourceGroupName "az104-rg4" -Location "eastus" -Name "myNSGSecure"

$vnet = Get-AzVirtualNetwork -ResourceGroupName "az104-rg4" -Name "ManufacturingVnet"
$subnet = Get-AzVirtualNetworkSubnetConfig -VirtualNetwork $vnet -Name "SensorSubnet1"
...
# Configure an NSG inbound security rule to allow ASG traffic

$inboundPorts = @('80', '443') 
$asg = Get-AzApplicationSecurityGroup -ResourceGroupName "az104-rg4" -Name "asg-web"
...
Add-AzNetworkSecurityRuleConfig -NetworkSecurityGroup $nsg `
    ...
    -DestinationPortRange $inboundPorts
...
$recordSetParams.DnsRecords = $aRecord
